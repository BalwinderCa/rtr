...
// RTF Generator Class 2.5 Build 150
// updated 11 october 2005
// Email: paggard@paggard.com

error_reporting  (E_ERROR | E_PARSE); // uncomment this if you wand to avoid "Warnings"
...
{\\f23\\fswiss\\fprq2\\fcharset".$default_charset."\\fprq2{\\*\\panose 02020404030301010958}Arial Narrow;}
		...
		$string = preg_replace("/[\r\n\t]+/","",$string);
			...
			if (preg_match_all("/(<font)(.*?)(>)/msi", $string, $fonts))
			{
				...
				for ($i = 0; $i < count($text); $i++) {
					parse_str(strtolower(preg_replace("/ +/i", "&", trim($fonts[$i]))));
					$perms = "";
					if (isset($face)) {
				...
				$string = preg_matchi_replace("</font>", $fig_r, $string);
			}
	//-----------------------------------------------------------------------------------------------
			if (preg_match_all("/(<a)(.*?)(>)/msi", $string, $links))
			{
				...
				for ($i=0;$i<sizeof($text);$i++) {
					...
					$links[$i] = preg_replace("&", "@@##@@", @trim($links[$i]));
					parse_str(strtolower(preg_replace(" +", "&", @trim($links[$i]))));
					...
					if ($local != "") {
						...
						$perms .= "HYPERLINK  ".$quote.preg_replace("@@##@@", "&", $file).$quote." ";
					}
				...
				$string = preg_matchi_replace("</a>", $slash."cf0".$fig_r.$fig_r, $string);
			}
...
//-----------------------------------------------------------------------------------------------
			...
			if (preg_match_all("/(<p)(.*?)(>)/msi", $string, $pars)) {
				...
				$string = "";
				for ($i=0;$i<sizeof($text);$i++) {
					parse_str(strtolower(preg_replace("/ +/", "&", @trim($pars[$i]))));
					if (isset($align)) {
...
// HR handle
			if (preg_match_all("/(<hr)(.*?)(>)/msi", $string, $hr_perms))
			{
				...
				$string = "";
				for ($i=0;$i<sizeof($text);$i++) {
					parse_str(strtolower(preg_replace(" +", "&", @trim($hr_perms[$i]))));
...
//-------------------------------------------------------------------------------------------------
		$string=preg_matchi_replace("<cpagenum>", $slash."chpgn ", $string);
		$string=preg_matchi_replace("\r", "", $string);$string=preg_matchi_replace("\n", "", $string);
		$string=preg_matchi_replace("<u>", "<U>", $string);$string=preg_matchi_replace("<U>", $slash."ul ", $string);
		$string=preg_matchi_replace("</u>", "</U>", $string);$string=preg_matchi_replace("</U>", $slash."ul0 ", $string);
		$string=preg_matchi_replace("<i>", "<I>", $string);$string=preg_matchi_replace("<I>", $slash."i ", $string);
		$string=preg_matchi_replace("</i>", "</I>", $string);$string=preg_matchi_replace("</I>", $slash."i0 ", $string);
		$string=preg_matchi_replace("<b>", "<B>", $string);$string=preg_matchi_replace("<B>", $slash."b ", $string);
		$string=preg_matchi_replace("</b>", "</B>", $string);$string=preg_matchi_replace("</B>", $slash."b0 ", $string);
		$string=preg_matchi_replace("<br>", "<BR>", $string);$string=preg_matchi_replace("<BR>", " ".$slash."line ", $string);
		$string=preg_matchi_replace("<sup>", "<SUP>", $string);$string=preg_matchi_replace("<SUP>", $fig_l.$slash."super ", $string);
		$string=preg_matchi_replace("</sup>", "</SUP>", $string);$string=preg_matchi_replace("</SUP>", $fig_r, $string);
		$string=preg_matchi_replace("<sub>", "<SUB>", $string);$string=preg_matchi_replace("<SUB>", $fig_l.$slash."sub ", $string);
		$string=preg_matchi_replace("</sub>", "</SUB>", $string);$string=preg_matchi_replace("</SUB>", $fig_r, $string);
		$string=preg_matchi_replace("<new page>"," ".$slash."page ", $string);
		$string=preg_matchi_replace("<newcol>"," ".$slash."column ", $string);
//-------------
//		$string=preg_matchi_replace("<header>", "<HEADER>", $string);$string=preg_matchi_replace("<HEADER>", $fig_l.$slash."header ".$slash."pard".$slash."plain ".$slash."q".$this->get_align($this->header_align)." ".$fig_l, $string);
//		$string=preg_matchi_replace("</header>", "</HEADER>", $string);$string=preg_matchi_replace("</HEADER>", $fig_r.$fig_l.$slash."par ".$fig_r.$fig_r, $string);


		$string=preg_matchi_replace("<header>", "<HEADER>", $string);$string=preg_matchi_replace("<HEADER>", $fig_l.$slash."header ".$slash."pard".$slash."plain ".$slash."q".$this->get_align($this->header_align)." ", $string);
		$string=preg_matchi_replace("</header>", "</HEADER>", $string);$string=preg_matchi_replace("</HEADER>", $slash."par ".$fig_r, $string);
//-------------
		$string=preg_matchi_replace("<footer>", "<FOOTER>", $string);$string=preg_matchi_replace("<FOOTER>", $fig_l.$slash."footer ".$slash."pard".$slash."plain ".$slash."q".$this->get_align($this->footer_align)." ".$fig_l, $string);
		$string=preg_matchi_replace("</footer>", "</FOOTER>", $string);$string=preg_matchi_replace("</FOOTER>", $fig_r.$fig_l.$slash."par ".$fig_r.$fig_r, $string);
// if facing_pages is activated:
		$string=preg_matchi_replace("<headerr>", "<HEADERR>", $string);$string=preg_matchi_replace("<HEADERR>", $fig_l.$slash."headerr ".$slash."pard".$slash."plain ".$slash."q".$this->get_align($this->header_align)." ", $string);
		$string=preg_matchi_replace("</headerr>", "</HEADERR>", $string);$string=preg_matchi_replace("</HEADERR>", $slash."par ".$fig_r, $string);
//-------------
		$string=preg_matchi_replace("<footerr>", "<FOOTERR>", $string);$string=preg_matchi_replace("<FOOTERR>", $fig_l.$slash."footerr ".$slash."pard".$slash."plain ".$slash."q".$this->get_align($this->footer_align)." ".$fig_l, $string);
		$string=preg_matchi_replace("</footerr>", "</FOOTERR>", $string);$string=preg_matchi_replace("</FOOTERR>", $fig_r.$fig_l.$slash."par ".$fig_r.$fig_r, $string);
//-------------
		$string=preg_matchi_replace("<headerl>", "<HEADERL>", $string);$string=preg_matchi_replace("<HEADERL>", $fig_l.$slash."headerl ".$slash."pard".$slash."plain ".$slash."q".$this->get_align($this->header_align)." ", $string);
		$string=preg_matchi_replace("</headerl>", "</HEADERL>", $string);$string=preg_matchi_replace("</HEADERL>", $slash."par ".$fig_r, $string);
//-------------
		$string=preg_matchi_replace("<footerl>", "<FOOTERL>", $string);$string=preg_matchi_replace("<FOOTERL>", $fig_l.$slash."footerl ".$slash."pard".$slash."plain ".$slash."q".$this->get_align($this->footer_align)." ".$fig_l, $string);
		$string=preg_matchi_replace("</footerl>", "</FOOTERL>", $string);$string=preg_matchi_replace("</FOOTERL>", $fig_r.$fig_l.$slash."par ".$fig_r.$fig_r, $string);





		$string=preg_matchi_replace("<tab>", "<TAB>", $string);$string=preg_matchi_replace("<TAB>", $slash."tab ", $string);
		$string=preg_matchi_replace("<hr>", "<HR>", $string);$string=preg_matchi_replace("<HR>", $slash."brdrb".$slash."brdrs".$slash."brdrw15".$slash."brsp20  ".$slash."par".$slash."pard ", $string);
...
// ------------------------------------------------------------------------------------------------
	function cr_test_table() {
		for ($i=0;$i<50000;$i++) {
			$tbl[$i][0] = "test $i";
			$tbl[$i][1] = "test2 $i";
			$tbl[$i][2] = "test3 $i";
		}
		return $this->add_tbl($tbl);
	} // end of function
// ------------------------------------------------------------------------------------------------
		...
		if (preg_match_all("/(<img )(.*?)(>)/msi", $string, $imgs))
		{
			...
			for ($i=0;$i<sizeof($text_mass);$i++) {
				if (isset($images[$i]) && strlen($images[$i]) > 5) {
					$img_mass[$im_fl] = $this->parce_image($images[$i]);
...
//				else {$final .= $this->toCyr($text_mass[$i]).$fin_mass[$i];}
		...
		if (preg_match_all("/".$token."/ms", $final,$count_i)) {
			...
			for ($i=0;$i<sizeof($count_i);$i++) {
				$fnd = $token.$i."nort";
				$final = preg_replace($fnd,$img_mass[$i],$final);
			}
		}
		$this->text .= $final;
	}// end of function
...
function openimage($image) {
    ...
    if ($fp) {
        while (!feof($fp)) {
            $cy .= @fread($fp, 1024);
            // You may want to remove the $sz condition, as it's not necessary anymore.
        }
        @fclose($fp);
        return bin2hex($cy);
    } else {
        // Handle the case when the file cannot be opened.
        // For example, you can return false or throw an exception.
        return false;
    }
}
...
//-------------------------------------------------------------------------------------------------
	function parce_image($image) {
		$perms = preg_replace("/ +/", "&", trim(preg_replace("/&/", "#@@#", $image)));
		parse_str($perms, $parsedVars);
		...
		//$src = explode(".",preg_replace("\"","",$img_src));
...
//-------------------------------------------------------------------------------------------------
		function parce_table_new($table) {
			...
			/////////////
			if (preg_match("/(\d+)%/", $tbl_width, $matches)) {
				$yyy = (int)$matches[1];
			...
			if (preg_match_all("/(<tr)(.*?)(<\/tr>)/msi", $other, $trs))
			{
				...
				for ($r=0;$r<sizeof($trs);$r++) {
					...
					$p_tr = "";
					$perms = preg_replace("/ +/", "&", @trim($tmp2[0]));
					$perms = strtolower($perms);
					parse_str($perms, $parsedVars);
					...
					if (preg_match_all("/(<td)(.*?)(<\/td>)/msi", $other2, $tds)) {
						...
						for ($d=0;$d<sizeof($tds);$d++) {
							...
							$p_td = "";
							$perms = preg_replace("/ +/", "&", @trim($tmp3[0]));
							$perms = strtolower($perms);
							parse_str($perms, $parsedVars);
...
//-------------------------------------------------------------------------------------------------
	function tbl_full($mass_head,$mass_body,$mass_wdth,$width) {
		...
		for ($i=0;$i<count($mass_wdth);$i++) { for ($b=0;$b<$width;$b++){ $shablon_mass[$i][$b] = "&nbsp;"; $fin_tbl_head[$i][$b] = $hhh;$fin_tbl_body[$i][$b] = $h;$fin_tbl_wdth[$i][$b] = $hh; } }
		$num_id = 0;
		for ($a=0;$a<count($mass_wdth);$a++)
		{
			$id = 0; // ����� ����� � ����
			for ($c=0;$c<$width;$c++) {
				if ($fin_tbl_body[$a][$c]==$h) {
						for ($lk=0;$lk<count($mass_wdth[$a][$id]);$lk++) {
							for ($kl=0;$kl<count($mass_wdth[$a][$id][$lk]);$kl++) {
